...
export default function SignUp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    setLoading(true);
    const { error } = await signUp(email, password, fullName, phone, role, address);
    setLoading(false);
    if (error) {
      setError(error);
    } else {
      ...
      <div className="max-w-md w-full">
        ...
        <div className="bg-paper rounded-lg p-8 border border-forest-dark">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {role === 'household' && (
              <div>
                <label className="block text-sm font-medium text-ink/70 mb-1.5">Address</label>
                <div className="relative">
                  <Home className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-ink/30" />
                  <input
                    type="text"
                    required
                    value={address}
